...
public class PrintTemplate {
    ...
    //批发交易小票模板
    public void tradeTemplate(MerchantTrade trade,  final PrintResultCallback printResultCallback){
        try{
            ...
            for (int i = 0; i < trade.getMerchantTradeGoodsList().size(); i++){
                MerchantTradeGoods goods = trade.getMerchantTradeGoodsList().get(i);
            ...
            mPrinter.printStr(Printer.AlignLeft, "支付方式：" + StringUtil.paddingLeft(String.valueOf(trade.getPayType()), ' ', 16));
            mPrinter.printStr(Printer.AlignLeft, "------------------------------");
            mPrinter.printStr(Printer.AlignLeft, "卖家名称： " + trade.getBuyerName());
            mPrinter.printStr(Printer.AlignLeft, "买家名称： " + trade.getSellerName());
